...
const CONJ = {
  "être": {
    ...
    "futur simple": { "1s":"serai","2s":"seras","3s":"sera","1p":"serons","2p":"serez","3p":"seront" },
    "conditionnel présent": { "1s":"serais","2s":"serais","3s":"serait","1p":"serions","2p":"seriez","3p":"seraient" }
  },
  "avoir": {
    "présent":   { "1s":"ai","2s":"as","3s":"a","1p":"avons","2p":"avez","3p":"ont" },
    "imparfait": { "1s":"avais","2s":"avais","3s":"avait","1p":"avions","2p":"aviez","3p":"avaient" },
    "futur simple": { "1s":"aurai","2s":"auras","3s":"aura","1p":"aurons","2p":"aurez","3p":"auront" },
    "conditionnel présent": { "1s":"aurais","2s":"aurais","3s":"aurait","1p":"aurions","2p":"auriez","3p":"auraient" }
  },
  "aller": {
    ...
    "futur simple": { "1s":"irai","2s":"iras","3s":"ira","1p":"irons","2p":"irez","3p":"iront" },
    "conditionnel présent": { "1s":"irais","2s":"irais","3s":"irait","1p":"irions","2p":"iriez","3p":"iraient" }
  },
  "finir": {
    "présent":   { "1s":"finis","2s":"finis","3s":"finit","1p":"finissons","2p":"finissez","3p":"finissent" },
    "imparfait": { "1s":"finissais","2s":"finissais","3s":"finissait","1p":"finissions","2p":"finissiez","3p":"finissaient" },
    "futur simple": { "1s":"finirai","2s":"finiras","3s":"finira","1p":"finirons","2p":"finirez","3p":"finiront" },
    "conditionnel présent": { "1s":"finirais","2s":"finirais","3s":"finirait","1p":"finirions","2p":"finiriez","3p":"finiraient" }
  },
  "aimer": {
    "présent":   { "1s":"aime","2s":"aimes","3s":"aime","1p":"aimons","2p":"aimez","3p":"aiment" },
    "imparfait": { "1s":"aimais","2s":"aimais","3s":"aimait","1p":"aimions","2p":"aimiez","3p":"aimaient" },
    "futur simple": { "1s":"aimerai","2s":"aimeras","3s":"aimera","1p":"aimerons","2p":"aimerez","3p":"aimeront" },
    "conditionnel présent": { "1s":"aimerais","2s":"aimerais","3s":"aimerait","1p":"aimerions","2p":"aimeriez","3p":"aimeraient" }
  },

  // Placeholders pour faciliter l’ajout (à compléter plus tard si besoin)
  "ouvrir": null, "partir": null, "tenir": null, "manger": null, "commencer": null
};
...
function renderSettings(){
  ...
  Object.keys(CONJ).forEach(v=>{
    ...
    wrap.innerHTML = `<input type="checkbox" id="${id}" ${settings.verbesActifs.includes(v)?"checked":""}/> ${v}`;
    verbsChecks.appendChild(wrap);
    document.getElementById(id).onchange = (e)=>{
      if(e.target.checked){ if(!settings.verbesActifs.includes(v)) settings.verbesActifs.push(v); }
      else{ settings.verbesActifs = settings.verbesActifs.filter(x=>x!==v); }
    };
  });
  ...
  TEMPS.forEach(t=>{
    const id = "tchk_"+t;
    const wrap = document.createElement("label");
    wrap.innerHTML = `<input type="checkbox" id="${id}" ${settings.tempsActifs.includes(t)?"checked":""}/> ${t}`;
    tensesChecks.appendChild(wrap);
    document.getElementById(id).onchange = (e)=>{
      if(e.target.checked){ if(!settings.tempsActifs.includes(t)) settings.tempsActifs.push(t); }
      else{ settings.tempsActifs = settings.tempsActifs.filter(x=>x!==t); }
    };
  });
...
// ======= Adaptation / Progress =======
function keyOf(v,t,pc){ // pc: "1s" etc.
...
function updateOnAnswer(k, ok){
  ...
  if(ok){
    ...
    n.interval = (n.streak===1 ? 1 : n.streak===2 ? 3 : Math.max(3, Math.round(n.interval * n.ease)));
  ...
  const due = new Date(); due.setDate(due.getDate() + n.interval);
...
function fillSelects(){
  libreVerbe.innerHTML = settings.verbesActifs.filter(v=>CONJ[v]).map(v=>`<option>${v}</option>`).join("");
  libreTemps.innerHTML = settings.tempsActifs.map(t=>`<option>${t}</option>`).join("");
}
fillSelects();

document.getElementById("libreLoad").onclick = ()=>{
  const v = libreVerbe.value, t = libreTemps.value;
  if(!CONJ[v] || !CONJ[v][t]){ alert("Verbe/temps non disponible."); return; }
...
document.getElementById("libreCheck").onclick = ()=>{
  ...
  const rows = PERSONNES.map(p=>{
    const inp = document.querySelector(`#libreGrid input[data-p="${p.code}"]`).value.trim().toLowerCase();
    const correct = renderTargetForm(p, v, t);
    const ok = normalize(inp) === normalize(correct);
    // mise à jour adaptative
    updateOnAnswer(keyOf(v,t,p.code), ok);

    return {p, inp, correct, ok};
  });
  ...
  corrBox.innerHTML = rows.map(r=>(
    `<div class="row">
       <span>${r.p.label}</span>
       <span>${highlight(r.inp, r.correct)}</span>
       <span>→ ${r.correct}</span>
     </div>`
  )).join("");
  updateDashboard();
...
startQuizBtn.onclick = ()=>{
  ...
  if(pool.length===0){ alert("Aucun item disponible. Vérifie les réglages."); return; }
...
function nextQuestion(pool){
  ...
  const pickFrom = (Math.random()<0.7 && duePool.length) ? duePool : (altPool.length? altPool : pool);
...
function validateCurrent(){
  ...
  if(ok){
    ...
    quizState.errors.push({
      sujet: it.p.label, verbe: it.v, temps: it.t, correct
    });
  }
  saveLS(LS_KEYS.progress, progress);
...
function endQuiz(){
  ...
  saveLS(LS_KEYS.last, { score: quizState.score, total: quizState.total, when: nowISO(), errors: quizState.errors });
  updateDashboard();
}
...
function setProgress(ratio){
  ...
  qProgress.style.setProperty("background", `linear-gradient(90deg, #34d399 ${Math.round(ratio*100)}%, transparent 0)`);
}
...
function renderTargetForm(p, v, t){
  ...
  const subj = (sujet==="je" && needsElision(forme)) ? "j’" : sujet;
  return `${subj} ${forme}`.trim();
}
...
function highlight(given, correct){
  ...
  for(let i=0;i<len;i++){
    ...
    const b = c[i] ?? "";
    if(a===b) out += a;
    else out += `<span style="background:#3f1d1d;border-radius:4px;padding:0 2px">${a||"•"}</span>`;
  }
  return out || "—";
}

// ======= Init dashboard badges (simple mock) =======
(function seedBadges(){
  const el = document.getElementById("badges");
  el.innerHTML = `
    <li>🎯 10/10</li>
    <li>📆 Série 3 jours</li>
    <li>⚡ Révision à l’heure</li>
  `;
})();
